      ...
      <section class="mx-auto max-w-5xl px-4 py-10">
        ...
        <div class="mt-6 grid lg:grid-cols-2 gap-8">
          <div class="card overflow-hidden">
            @if (p.imageUrl) {
              <img [src]="p.imageUrl" [alt]="p.name" class="w-full aspect-square object-cover" />
            } @else {
              <div
          ...
          <div>
            <div class="flex flex-wrap gap-2">
              <app-badge tone="brand">{{ humanLabel(p.category.name) }}</app-badge>
              <app-badge tone="neutral">{{ humanLabel(p.type) }}</app-badge>
              @if (p.status === 'DISPONIBLE') {
                ...
                <app-badge tone="danger">Descontinuado</app-badge>
              }
            </div>
            <h1 class="mt-3 text-3xl">{{ p.name }}</h1>
            <p class="mt-2 text-[var(--color-ink-500)]">{{ p.shortDescription }}</p>
            ...
            <div class="mt-6 card p-5">
              <div class="flex items-baseline justify-between">
                <div>
                  ...
                  <div class="text-3xl font-semibold text-[var(--color-ink-900)]">
                    {{ p.price | currency: 'PEN' : 'symbol-narrow' : '1.2-2' : 'es-PE' }}
                  </div>
                  @if (p.unit) {
                    <div class="text-xs text-[var(--color-ink-500)]">por {{ p.unit }}</div>
                  }
                </div>
              ...
              <div class="mt-4 grid sm:grid-cols-2 gap-2">
                <app-button [fullWidth]="true" [disabled]="p.status !== 'DISPONIBLE'">
                  <app-icon name="shopping-bag" [size]="16" />
                  Agregar al carrito
                </app-button>
                <a [routerLink]="['/experts', p.seller.id]" class="block">
            ...
            <div class="mt-6 flex items-center gap-3 text-sm">
              <span
                ...
                aria-hidden="true"
              >
                {{ initials(p.seller.firstName, p.seller.lastName) }}
              </span>
              <div>
                ...
                <div class="text-xs text-[var(--color-ink-500)]">
                  {{ p.seller.specialty ?? 'Experto QaliKay' }}
                </div>
              </div>
            </div>
          </div>
        ...
        @if (p.description) {
          <article class="mt-10 card p-6 max-w-3xl">
            ...
            <p class="mt-2 text-[var(--color-ink-700)] whitespace-pre-line">{{ p.description }}</p>
          </article>
        }
      </section>
    }
...
export class ProductDetailPage implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!Number.isFinite(productId)) {
      this.errorMessage.set('Identificador de insumo inválido.');
      ...
      .pipe(
        catchError((err) => {
          ...
          if (status === 404) this.errorMessage.set('Este insumo no existe.');
            ...
            this.errorMessage.set('No pudimos contactar al servidor. ¿El backend está corriendo?');
          else this.errorMessage.set('Ocurrió un error inesperado al cargar el insumo.');
          this.loading.set(false);
          return of(null);
        }),
